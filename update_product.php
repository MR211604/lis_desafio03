...
$sql = "select * from usuarios where email = '$email' and id != '$update_id'";
$result = mysqli_query($conn, $sql);
if (mysqli_num_rows($result) > 0)
	$err_msg = "El usuario ingresado ya existe";

else {
	$sql = "update usuarios set name = '$name',password = '$password',email = '$email', birthdate = '$birthdate' where id='$update_id'";
	$result = mysqli_query($conn, $sql);
	if ($result) {
		$succ_msg = "Usuario actualizado correctamente";
		$_SESSION['succ_msg'] = $succ_msg;
	} else {
		$err_msg = "Error: No se pudo actualizar el usuario";
